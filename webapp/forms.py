...
class RegistrationForm(FlaskForm):
    area_number = StringField(label='Введите номер участка', validators=[DataRequired(message='Поле не должно быть пустым')],
    ...
    phone = TelField(label='Введите номер телефона', validators=[DataRequired(message='Поле не должно быть пустым')],
                                render_kw={'class': 'form-control'})
    password = PasswordField(label='Введите пароль', validators=[DataRequired(), EqualTo('confirm', 'Пороли должны совпадать')],
                                render_kw={'class': 'form-control'})
